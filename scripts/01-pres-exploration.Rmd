...
## The Data
This experiment explored the growth of populations of oysters taken from different locations (each with three sites) all grown in the same lab environment.
...
## PS_Larvae Data
This is the layout of the data table used for the following  scatterplots.
```{r, echo = FALSE}
...
These scatterplots break down the last one, instead showing the average length of oyster for each of the three populations at each of the three sites.
```{r, message = FALSE, echo = FALSE}
popul_days <- group_by(PS_larvae, Population, Date)
```

```{r, echo = TRUE}
#separated Replicate column to make location and site number columns to use for filtering and later scatterplots
...
f1_mean <- aggregate(.~number, data = fidalgo_bay$Average_Length, mean)
...
daysvslength_fidalgo <- ggplot(data = fidalgo_mean, mapping = aes(x = Days.after.start, y = mean_l, group = number)) +
